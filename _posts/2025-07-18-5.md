...
// ... (다른 import 및 AuthState 인터페이스 선언 생략) ...
...
function createAuthStore() {
  ...
  return {
    ...
    async login() {
      // ... (isLoading 상태 업데이트, googleAuth.signIn() 호출) ...
      // ... (성공 시 localStorage에 사용자 정보 저장 및 상태 업데이트) ...
      // ... (실패 시 에러 상태 업데이트) ...
    },

    async logout() {
      // ... (isLoading 상태 업데이트, googleAuth.signOut() 호출) ...
      // ... (localStorage에서 사용자 정보 제거 및 상태 업데이트) ...
    },

    async checkAuthStatus() {
      // ... (페이지 로드 시 localStorage를 확인하여 로그인 상태 복원) ...
    },
...
<script lang="ts">
  ...
  // ... (변수 선언 및 onMount, 핸들러 함수 생략) ...
  // onMount: authStore.checkAuthStatus() 호출 및 외부 클릭 리스너 등록
  // handleLogin, handleLogout: authStore의 메서드 호출
  // toggleDropdown, clearError: UI 상태 변경
</script>
...
<style>
  /* ... (약 200줄의 CSS 스타일링 생략) ... */
  /* 주요 스타일: .auth-container, .login-button, .spinner, .user-profile 등 */
  /* 반응형 디자인을 위한 미디어 쿼리 포함 */
</style>
```
...
// ... (GoogleUser 인터페이스 및 상수 선언 생략) ...

class GoogleAuthService {
  private isInitialized = false;

  async initialize(): Promise<void> {
    // ... (Google Identity Services 스크립트 동적 로딩 및 초기화) ...
  }

  private loadGoogleIdentityScript(): Promise<void> {
    // ... (스크립트 태그를 동적으로 생성하여 gsi/client 로드) ...
  }

  async signIn(): Promise<GoogleUser> {
    // ... (토큰 클라이언트 초기화 및 액세스 토큰 요청) ...
    // - 성공 시: 액세스 토큰으로 사용자 정보 API 호출 후 GoogleUser 객체 반환
    // - 실패 시: 에러 타입에 따라 적절한 에러 메시지와 함께 reject
  }

  async signOut(): Promise<void> {
    // ... (로컬 스토리지 정리 등 로그아웃 처리) ...
  }

  isSignedIn(): boolean {
    // ... (localStorage를 확인하여 로그인 상태 반환) ...
  }

  getCurrentUser(): GoogleUser | null {
    // ... (localStorage에서 사용자 정보를 가져와 반환) ...
  }
}
