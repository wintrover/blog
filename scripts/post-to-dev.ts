...
export function absolutizeSrc(src: string, publicBaseUrl: string) {
  try {
    ...
    if (trimmed.includes('ufileos.com') || trimmed.includes('UCloudPublicKey') || trimmed.includes('signature=') || trimmed.includes('Expires=')) {
...
export async function absolutizeImagesInMarkdown(markdown: string, publicBaseUrl: string, firstImageRef: { url: string | null }) {
...
async function postToDev(filePath: string) {
  ...
  try {
    ...
    const normalizedTags = Array.isArray(rawTags) ? rawTags : typeof rawTags === 'string' ? rawTags.split(/[\,\s]+/).filter(Boolean) : []
    const sanitized = Array.from(new Set(normalizedTags.map((t: any) => String(t).toLowerCase().replace(/[^a-z0-9]/g, '')).filter(Boolean)))
    ...
    const datePart = (m && m[1]) || ((frontmatter as any).date || '').toString()
    const numPart = (m && m[2]) || '0'
    const filenameBase = `${datePart}-${numPart}`
    const { content: processedContent, images: mermaidImages } = await processMermaidDiagrams(content, publicBaseUrl, mermaidOutputDir, filenameBase)
    if (mermaidImages.length > 0) {
      console.log(`✅ Successfully converted ${mermaidImages.length} Mermaid diagram(s) to images`)
      mermaidImages.forEach(img => { console.log(`   📊 ${img.filename} -> ${img.url}`) })
    } else {
    ...
    const slug = slugifyTitle((frontmatter as any).title || path.basename(filePath, path.extname(filePath)))
    const canonicalUrl = (frontmatter as any).canonical_url || `${publicBaseUrl}#/post/${slug}`
    const clampTitle = (t: string) => { if (!t) return ''; return t.length <= 128 ? t : (t.slice(0, 125) + '...') }
    const article = {
      title: clampTitle((frontmatter as any).title),
      published: false,
      body_markdown: bodyMarkdown,
      tags,
      description: (frontmatter as any).excerpt || (frontmatter as any).description || undefined,
...
async function downloadAndHostUCloudImage(ucloudUrl: string, publicBaseUrl: string) {
  try {
...
function isUCloudUrl(url: string) {
  return url.includes('ufileos.com') || url.includes('UCloudPublicKey') || url.includes('signature=') || url.includes('Expires=')
}
