...
export async function convertMermaidToImage(mermaidCode: string, outputPath: string) {
...
${mermaidCode}
    ...
    await mermaidElement.screenshot({ path: outputPath, type: 'png', omitBackground: false })
    return outputPath
  } catch (error) {
...
export function extractMermaidBlocks(markdown: string) {
  ...
  while ((match = mermaidRegex.exec(markdown)) !== null) {
    mermaidBlocks.push({ code: match[1].trim(), startIndex: match.index, endIndex: match.index + match[0].length })
  }
  return mermaidBlocks
}

export async function processMermaidDiagrams(markdown: string, publicBaseUrl: string, outputDir = 'public/images', filenameBase = 'diagram') {
  const mermaidBlocks = extractMermaidBlocks(markdown)
  if (mermaidBlocks.length === 0) {
    return { content: markdown, images: [] as { path: string; url: string; filename: string }[] }
  }
  ...
  for (let i = mermaidBlocks.length - 1; i >= 0; i--) {
    ...
    try {
      ...
      processedContent = processedContent.substring(0, block.startIndex) + imageMarkdown + processedContent.substring(block.endIndex)
      ...
      processedContent = processedContent.substring(0, block.startIndex) + fallbackMarkdown + processedContent.substring(block.endIndex)
    }
  }
